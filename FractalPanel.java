...
public class FractalPanel extends JPanel {
	...
	public void paintComponent( Graphics g ) {
		 ...
		 * 
		 */
		for(int i = 0; i < lines.size(); i++){ // iterate through the ArrayList
			
			
			drawKochLine(g2, level, (KochLine)lines.get(i) ); //call drawKochLine( Graphics2D g2, int level, KochLine line ) 
		}
	...
	public void drawKochLine( Graphics2D g2, int level, KochLine line ) {
		...
		if(level == 0){ // check to see if level is equal to 0
			...
			if(line.isKochLineValid() == true){
				...
				line.getLineD().getEnd().getY()); //this will draw these lines if level == 0
				
			}
		}
		else{ // if level is not equal to zero
